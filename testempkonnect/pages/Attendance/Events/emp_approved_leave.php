...
function getLeaves($month,$year,$emp_code){
	...
	$result1= array();
	$sqlq="SELECT cast(LvFrom as date) as startDate,cast(LvTo as date) as endDate,status,reason FROM leave WHERE leaveID IN (SELECT MAX(leaveID) FROM leave GROUP BY LevKey) AND (LvFROM BETWEEN '".$firstDate."' AND '".$lastDate."' OR LvTO BETWEEN '".$firstDate."' AND '".$lastDate."') and createdby='".$emp_code."'
ORDER BY LvFrom";
	...
	if($tempArray4>0) {
		while ($rowq = $fetch($resultq)){
			...
			for($i = $startFrom; $i<=$endTo; $i++){
				$varArray[] = array('date'=>$year.'-'.$month.'-'.sprintf("%02d", $i),'status'=>$status,'reason'=>$rowq['reason']);
			}
		}
		//return $result1;
	}
	...
	for($j=0; $j<count($varArray); $j++){
		if($varArray[$j]['status']=='Approved'){
			$result1[] = array('type'=>'leave','title'=>'Leave','reason'=>$varArray[$j]['reason'],'status'=>$varArray[$j]['status'],'start'=>$varArray[$j]['date'],'className'=>array("pink_over circle-legend"),'presetShortStatus'=>'P');
		}else if($varArray[$j]['status']=='Pending'){
			$result1[] = array('type'=>'leave','title'=>'Leave','reason'=>$varArray[$j]['reason'],'status'=>$varArray[$j]['status'],'start'=>$varArray[$j]['date'],'className'=>array("blue_over circle-legend"),'presetShortStatus'=>'A');
		}else{
			$result1[] = array('type'=>'leave','title'=>'Leave','reason'=>$varArray[$j]['reason'],'status'=>'Absent','start'=>$varArray[$j]['date'],'className'=>array("red circle-legend"),'presetShortStatus'=>'A');
		}
	}
	...
	if($tempArray41>0) {
		...
		for($j=0; $j<count($varArray); $j++){
			if($varArray[$j]['status']=='Approved'){
				...
				$result[] = array('type'=>'attRegularised','title'=>'Attendance Regularised','reason'=>$varArray[$j]['reason'],'status'=>$varArray[$j]['status'],'start'=>$varArray[$j]['date'],'className'=>array("yellow_over circle-legend"),'presetShortStatus'=>'A','startTime'=>$varArray[$j]['startTime'],'endTime'=>$varArray[$j]['endTime']);
			}else{
	...
	if($tempArray41>0) {
		...
		for($j=0; $j<count($varArray); $j++){
			if($varArray[$j]['status']=='Approved'){
				...
				$result[] = array('type'=>'odrequest','title'=>'OD Request','reason'=>$varArray[$j]['reason'],'status'=>$varArray[$j]['status'],'start'=>$varArray[$j]['date'],'className'=>array("pink circle-legend"),'presetShortStatus'=>'A','startTime'=>$varArray[$j]['startTime'],'endTime'=>$varArray[$j]['endTime']);
			}else{
