...
$c_val["Leave Management"]["My Comp off Request"] = 'end1234';
//$c_val = 'end1234';
?>
<!DOCTYPE html>
...
<div class="page-content-wrapper cus-dark-grey">
    ...
    <div class="page-container">
        ...
        <!-- END SIDEBAR -->
        <!-- BEGIN CONTENT -->
        <?php include ('content/myCompOffRequest_content.php');?>
...
<script>
    ...
    function getmycompoffId(mycoId,status,code){

        $.ajax({
            type: "POST",
            url: "content/view_compOffPopup.php",
            data: {id: mycoId, status:status, code:code},
            success: function (result) {
                $('#mycompoff').html(result);
            }
        });
    }


    function submitCancelRequest(coId,code){
        var type="usercancel";
        var remrk = $("#remrk").val();
       // alert(remark);
      //  alert(coId);
        ...
        //alert(type);
        if(remrk == ''){
            toasterrormsg("Required Remarks for cancellation");
            return false;
        }
        $.ajax({
            type: "POST",
            url: "ajax/compOff_ajax.php",
            data: {id: coId, type:type, code:code, remrk:remrk},
            success: function (result) {
                if(result == 1){

                   toastmsg("Applied Request Cancelled");
                   location.reload();
                }else{
                    toasterrormsg("Error in cancellation");
                }
            }
        });
    }
    ...
    function searchByDate(code){
        ...
        $.ajax({
            type:"POST",
            url:"ajax/compOff_ajax.php",
            data:{type:type, fd:fd, td:td, code:code},
            success: function(result){
                if(result){
                    //location.reload();
