...
if(isset($_GET['type']) && $_GET['type']=='data1'){
    ...
    $sqlq="select * from WorkFlow WHERE WFFor='Leave' and WEvents='ALL' order by WorkFlowID DESC ";
    //echo $sqlq;
    $resultq = query($query,$sqlq,$pa,$opt,$ms_db);
    {
        while($row = $fetch($resultq)) {

            $sql="Select Emp_Code from hrdmastQry";
...
else if(isset($_GET['type']) && $_GET['type']=='data3'){
    $workflow = Array();
    $level= Array();
    $sqlq="select * from WorkFlow WHERE WFFor='Leave' and WEvents='Advance Leave' order by WorkFlowID DESC ";
    $resultq = query($query,$sqlq,$pa,$opt,$ms_db);
    {
        while($row = $fetch($resultq)) {
...
elseif (isset($_GET['type']) && $_GET['type']=='data2') {
    if($_POST['dataval'] == ""){
        ...
        $lev = explode(',',$_POST['dataval']);
         echo '<input type="hidden" id="approver_leave_approver1" value="'.count($lev).'">';
    for($i=0;$i<count($lev);$i++){
        if($lev[$i]=='Reporting Manager'){
            ...
            while($rowr = $fetch($resultr)){
                ...
                echo      "  </span>";
                echo    "    <span class='appManName' data-des='".str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row1['DSG_NAME']))))."'>";
                                echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row1['EMP_NAME'])))); 
        ...
        if($lev[$i]=='Functional Manager'){
            ...
            while($rowf = $fetch($resultf)){
                ...
                while($row2 = $fetch($result2)){
                    ...
                    echo      "  </span>";
                    echo    "    <span class='appManName' data-des='".str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row2['DSG_NAME']))))."'>";  
                    echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row2['EMP_NAME']))));  
                     echo"</span></span>";
        ...
        if($lev[$i]=='Function Head'){
            ...
            while($rowfh = $fetch($resultfh)){
                ...
                while($row3 = $fetch($result3)){
                    ...
                    echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row3['EMP_NAME'])))); echo "<i> "; echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row3['DSG_NAME'])))); echo "</i>";
                     echo"</span></span>";
            ...
            while($rowb = $fetch($resultb)){
                ...
                while($row4 = $fetch($result4)){
                    echo "<span class='appMan blue-bg'>
                     ...
                     echo      "  </span>";
                    echo    "    <span class='appManName'>";
                    echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row4['EMP_NAME'])))); echo "<i> "; echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row4['DSG_NAME'])))); echo"</i>"; 
                     echo"</span></span>";
        ...
        if($lev[$i]=='Sub Functional Head'){
            ...
            while($rowsf = $fetch($resultsf)){
                ...
                while($row5 = $fetch($result5)){
                    ...
                    echo      "  </span>";
                    echo    "    <span class='appManName'>";
                    echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row5['EMP_NAME'])))); echo "<i>"; echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row5['DSG_NAME'])))); echo"</i>";
                     echo"</span></span>";
        ...
        if($lev[$i]=='Sub Bussiness Head'){
            ...
            while($rowsb = $fetch($resultsb)){
                ...
                while($row6 = $fetch($result6)){
                    ...
                    echo      "  </span>";
                    echo    "    <span class='appManName' data-des='".str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row6['DSG_NAME']))))."'>";
                    echo str_replace('\' ', '\'', ucwords(str_replace('\'', '\' ', strtolower($row6['EMP_NAME'])))); 
                     echo"</span></span>";
     ...
     $approver_leave_ids = explode(',',$appID);
        //echo $approver_leave_ids."aaa".count($level_ids);
        echo '<input type="hidden" id="approver_leave_ids1" value="'.count($approver_leave_ids).'">';

    echo'<input type="hidden" id="approverId" value="'.$appID.'">';
    }
    
}
elseif (isset($_GET['type']) && $_GET['type']=='data4') {
    if($_POST['dataval'] == ""){
        echo "Manager is not Available";
    }else{

        $lev = explode(',',$_POST['dataval']);
       // echo $_POST['dataval']."aaa".count($lev);
        echo '<input type="hidden" id="approver_leave_approver" value="'.count($lev).'">';
    for($i=0;$i<count($lev);$i++){
        if($lev[$i]=='Reporting Manager'){
            $sqlr="Select * from hrdmastQry where Emp_Code='$code'";
            $resultr = query($query,$sqlr,$pa,$opt,$ms_db);
...
$approver_leave_ids = explode(',',$appID);
        //echo $approver_leave_ids."aaa".count($level_ids);
        echo '<input type="hidden" id="approver_leave_ids" value="'.count($approver_leave_ids).'">';
    echo'<input type="hidden" id="approverId" value="'.$appID.'">';
    }
