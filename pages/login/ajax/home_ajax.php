...
//get matched data from skills table
    $sqlq = "SELECT * FROM EmpNameQry WHERE (Emp_Code LIKE '%" . $searchTerm . "%') OR (EMP_NAME LIKE '%" . $searchTerm . "%') OR (DEPT_NAME  LIKE '%" . $searchTerm . "%') OR (DSG_NAME  LIKE '%" . $searchTerm . "%') ORDER BY EMP_NAME ASC";
...
elseif ($flag_value == '2' ) {
    ...
    echo '<script src="../login/js/home.js"></script><table class="table table-hover table-light">';
//get matched data from skills table
    $sqlq="SELECT * FROM EmpNameQry WHERE (DOJ between DATEADD(DD,-30,GETDATE()) and GETDATE()) AND((Emp_Code LIKE '%".$searchTerm."%') OR (EMP_NAME LIKE '%".$searchTerm."%')) ORDER BY EMP_NAME ASC";
    $resultq = query($query, $sqlq, $pa, $opt, $ms_db);
...
elseif ($flag_value == '7'){
    ...
    wishmailer('donotreply@sequelone.com',$name,$subject,$message,$send,'');
