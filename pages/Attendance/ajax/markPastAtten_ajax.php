...
else if ($_POST['type'] == "getInOutTime") {
  extract($_POST);
  $dateval=explode('/',$fromDate);
  $fromDateConverted=$dateval[2].'-'.$dateval[1].'-'.$dateval[0];
  $sql="select IN_TIME, OUT_TIME from cattendanceqry WHERE EMP_CODE='".$code."' AND ATTDATE = '".$fromDateConverted."'";
  $res=query($query,$sql,$pa,$opt,$ms_db);
  $inOutTime = array();
  if($res){
    $result = $fetch($res);
    $inOutTime['intime'] = (isset($result['IN_TIME']))?date('h:i:a',strtotime($result['IN_TIME'])):'';
    $inOutTime['outtime'] = (isset($result['OUT_TIME']))?date('h:i:A',strtotime($result['OUT_TIME'])):'';
  }
  echo json_encode($inOutTime);
}
