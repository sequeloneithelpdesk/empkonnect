...
if($status_flag==3)
{
  ...
  for($i=0;$i<sizeof($leave_id);$i++)
  {
    ...
    $leaveApproveRequestWhenReject=$leave_class_obj->leaveApproveRequestWhenReject($leave_id[$i],$status_flag,$remark,$i,$planned);
...
else if($status_flag==2)
{
  ...
  for($i=0;$i<sizeof($leave_id);$i++)
  {
   // echo "qqqq".$leave_id[$i];
     $leaveApproveRequestWhenApprove=$leave_class_obj->leaveApproveRequestWhenApprove($leave_id[$i],$status_flag,$i,$planned);
