...
if($key=='EL'){
	...
	if($value['name']=='BTL'){
    	...
    	$Leave_Name='Birthday Leave/Anniversary Leave';
    }
	}
	else{
	...
	$leave_b.="<tr><td>".$Leave_Name."</td><td style='text-align: center;'>";
	$leave_b.=$opening;  
	$leave_b.="</td><td style='text-align: center;'>". $accur ."</td><td style='text-align: center;'>".$avail."</td><td style='text-align: center;'>".$totall_day."</td><td style='text-align: center;'></td><td style='text-align: center;'>".$totall_day."</td></tr>";
...
if($key=='EL'){
	...
	$totall_day=getCL_SL($key,$value['day_limit'],$opening,$avail,$value['rules']['accumulation'],$DoL,$DoJ,$DOJ_WEF);
$leave_b.="<div class='leaveCon clearfix'><div class='col-md-9'>".$Leave_Name."</div><div class='col-md-2' style='float: right;' id='".$key."'>".$totall_day."</div></div>";
...
$holiday.='<div class="leaveCon clearfix" ><div class="col-md-8"> '.$datapast[1].'</div> <div class="col-md-4"><div style="float:right;">'.$datapast[0].' </div></div></div>';
...
/* sachin */
else if($_REQUEST['type']=="getAllEventsTypes"){
	$id=$_POST['id'];
	//echo "sss";
	//$json = json_decode($data, true);
	//print_r($data);
	$alltypes="<div class='z-depth-1'>";
	foreach ($data[$employeetype][0] as $key => $value) {
	# code...
	//$avail=3;
	 $sql4="Select REPLACE(CONVERT(VARCHAR(10),a.DOJ,102),'.','-') AS DOJ,REPLACE(CONVERT(VARCHAR(10),a.DOJ_WEF,102),'.','-') AS DOJ_WEF,REPLACE(CONVERT(VARCHAR(10),a.DOB,102),'.','-') AS DOB,a.MStatus,REPLACE(CONVERT(VARCHAR(10),a.DOM,102),'.','-') AS DOM,a.SEX from hrdmastqry a where emp_code='$id'";
...
$a=1;
	$sqlpolicy="select Sub_Policy_Name,status,PolicyContent,CONVERT (VARCHAR(10),effective_date,103 ) as effective_date from hrpolicy where CONVERT(date,GETDATE(),3) >=effective_date and Sub_Policy_Name='$Leave_Name' and status='1'";
	//echo $sqlpolicy;
	$result2=query($query, $sqlpolicy, $pa, $opt, $ms_db);
	 if($row2=$fetch($result2))
	 {
    $PolicyContent=$row2['PolicyContent'];
	//echo $sqlpolicy;
//	$totall_day=getCL_SL($key,$value['day_limit'],$opening,$avail,$value['rules']['accumulation'],$DoL,$DoJ,$DOJ_WEF);
$alltypes.="<div class='leaveCon clearfix'><div class='col-md-9'>".$Leave_Name."</div><div class='col-md-2' style='float: right;' id='".$a."'><a href='../ABC_uploads/".$PolicyContent."' >View</a></div></div>";
	}	
...
$alltypes.="</div>";
	...
	/*$sqlpolicy="select Sub_Policy_Name,status,CONVERT (VARCHAR(10),effective_date,103 ) as effective_date from hrpolicy";
	//echo $sqlpolicy;
 	$respolicy=query($query,$sqlpolicy,$pa,$opt,$ms_db);
 	//var_dump($respolicy);
 	//echo $respolicy;
 	while ($datapolicy=$fetch($respolicy))
 	{
 	
 		echo $datapolicy['Sub_Policy_Name'];
    }
