...
if (isset($_REQUEST['planned']) && !empty($_REQUEST['planned']))
{
...
if (isset($_REQUEST['reasoncombo']) && !empty($_REQUEST['reasoncombo']))
{

  $reasoncombo=$_REQUEST['reasoncombo'];

}
if (isset($_REQUEST['reasontext']) && !empty($_REQUEST['reasontext']))
{
...
if($status_flag==3)
{
  ...
  for($i=0;$i<sizeof($leave_id);$i++)
  {
    ...
    $leaveApproveRequestWhenReject=$leave_class_obj->leaveApproveRequestWhenReject($leave_id[$i],$status_flag,$remark,$i,$planned,$reasoncombo,$reasontext);
...
else if($status_flag==2)
{
  ...
  for($i=0;$i<sizeof($leave_id);$i++)
  {
   // echo "qqqq".$leave_id[$i];
     $leaveApproveRequestWhenApprove=$leave_class_obj->leaveApproveRequestWhenApprove($leave_id[$i],$status_flag,$i,$planned,$reasoncombo,$reasontext);
...
else if($status_flag==4)
{
  ...
  {
    //echo "bbbb".$leave_id[$i];
    $leaveApproveRequestWhenCancel=$leave_class_obj->leaveApproveRequestWhenCancel($leave_id[$i],$status_flag,$i);
           $temp[$count]=$leaveApproveRequestWhenCancel;
          // print_r($leaveApproveRequestWhenReject);
    $count++;
  }
  //print_r($temp);
