...
and CONVERT (date,updation_date,103) <=convert(date, '$end',103) group by Levkey,Createdby,LvDays) a";
  //echo $sql2;
   ...
   function getemployee_name($code){
    ...
    function getemployee_category($code){
 ...
 function cancelLeaveRequestWhenBothNotApproved($leaveIDs,$ApprovedBy,$remark,$lvkey){
  ...
  function reApplyLeaveRequestCanceled($leaveIDs,$ApprovedBy,$lvkey){
   ...
   function leaveApproveRequestWhenReject($leaveID,$flag,$remark,$t,$planned,$reasoncombo,$reasonText){
     ...
     $var=array();
        ...
        //echo $sql;
        $update_sql = "UPDATE leave SET status='3',remark='$remark',planned_unplanned='$planned',leaveReason='$reasoncombo',reasonText ='$reasonText',updation_date=getdate() WHERE levkey='$lvkey'";
        $res1=query($query,$update_sql,$pa,$opt,$ms_db);
        //echo  $update_sql;
  ...
  function leaveApproveRequestWhenApprove($leaveID,$flag,$t,$planned,$reasoncombo,$reasonText){
    ...
    $var=array();
        ...
        $ApprovedBy=$row4['ApprovedBy'];
         $lvkey=$row4['levKey'];
        //echo $sql;
        $update_sql = "UPDATE leave SET status='2',planned_unplanned='$planned',leaveReason='$reasoncombo',reasonText ='$reasonText',updation_date=getdate() WHERE leaveID='$leaveID'";
        $res1=query($query,$update_sql,$pa,$opt,$ms_db);
//echo $update_sql;
