  ...
  if(new Date(forDate).getTime() == new Date(toDate).getTime())
  {
   // alert("1");
...
//alert(approverId);
        ...
        if ($("#dateCriteriadiv").val() == 0) {

            $("#fromDate").css('border-color', 'red');
             toasterrormsg("You can not apply more than 30 or less than 30 days ");
            return false;
        }else{
            $("#fromDate").css('border-color', '');
        }
        ...
        if(leaveTypes == 3){
     //  alert(noOfDays+"upload_value.length="+upload_value.length);
        var fuData = document.getElementById('uploadfile');
        var FileUploadPath = fuData.value;
       if (FileUploadPath == '')
          if(noOfDays >=2  && FileUploadPath ==''){
           //alert("true");
                $("#leaveErr").html("Please provide attachments.");
                 toasterrormsg("Please provide attachments.");
                $("#uploadMultiple").show();
		...
		return false;
		 ...
		 var foo = []; 
        $('#notificationto :selected').each(function(i, selected){ 
          foo[i] = $(selected).val(); 
        });
      //  alert(foo);
        ...
        if(flag == 0){
            ...
            formData1.append('empEmail',empEmail);
	    formData1.append('foo',foo);
...
//alert(l[2]);
    ...
    function getLeaveBalance(lvtype,ecode){
       ...
       // alert(applDays);
        var diff = Earned-applDays;
       // alert(diff+"elused"+elused);
        if(lvtype == 1)
        {
         //alert("a");
          if(elused == '2' || parseFloat(diff)< -15)
          {
       ...
       //   var max = parseFloat("10").toFixed(2);
          ...
          {
             //alert("applDays22"+applDays);
            $("#leaveErr").html("Should be less than 2 or equal 2 leave Casual Leave");
             toasterrormsg("Should be less than 2 or equal 2 leave Casual Leave");
           $("#leaveSubmit").prop('disabled', true);
            $("#usenextleave").hide();
            $("#uploadMultiple").hide();
          }
         ...
         //alert(applDays+ "qqq"+parseInt(SL.innerHTML));
        // alert(SL);
            $("#leaveErr").html("");
            $("#usenextleave").hide("");
            $("#leaveSubmit").show();
            if(applDays > parseInt(SL.innerHTML))
            {
          //  alert("aaa");
               $("#leaveErr").html("Don't have Enough Balance for This Leave");
              $("#uploadMultiple").hide();
       $("#leaveSubmit").prop('disabled', true);
            ...
            if(applDays >= 2 ){
           // alert(applDays+"aaa"+SL.innerHTML);
...
function myAttendanceByDAte(id,from,to){
 ...
 var forDate=document.getElementById(from).value;
  ...
  function check_approver_ids() 
  {
   //alert("aaa");
     ...
     // alert("true");
        ...
        toasterrormsg("You don't have enough approver. Please contact HR");
        $("#leaveSubmit").prop('disabled', true);
     }
     else
     {
  ...
  function checkDateCriteria()
    {
      var forDate=$("#fromDate").val();
      var toDate=$("#toDate").val();
    //  alert(forDate+toDate);
     // alert("cc");
      $.ajax({
            type: "POST",
            url: "ajax/dateCriteria.php",
            data: {forDate:forDate, toDate:toDate },
            cache:false,
             dataType: "json",
            success: function (result) {
            // alert(result);
              ...
              {
                    $("#err_attendance").html("");
                    $('#dateCriteriadiv').val(result);
                    $("#leaveSubmit").prop('disabled',false);
              }
              else
              {
                $("#err_attendance").html("You can not apply more than 30 or less than 30 days");
                $('#dateCriteriadiv').val(result);
               $("#leaveSubmit").prop('disabled', true);
              }
