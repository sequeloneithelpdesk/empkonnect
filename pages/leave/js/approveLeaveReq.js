  ...
  function submitApprovelRequest(id,status_flag,del){
   alert(del);
    if(del==1)
    {
        ...
        var radio_action=$("input[name='radio_action']:checked").val();
        var val1=$("#LeaveReasonscombo").val();
         var a= $("#othercombo").val();
       // alert(val1+a);
        if(typeof radio_action=="undefined")
        	{
        	 toasterrormsg("Please Select Action");
             return false;
        	}
       // alert(radio_action);
        if(radio_action==0)
        {
              if(val1==0)
              {
                 toasterrormsg("Please Select Unplanned Reason");
                 return false;
              }
              if(val1==6 && a=="")
              {
                 toasterrormsg("Please Enter Your Unplanned Reason");
                 return false;
              }
        }
        ...
        if(status_flag==3){
             ...
             if(remark=="")
             {
                 $("#actRemarks").addClass('errorRed');
                toasterrormsg("Please Enter Remark");
    ...
    else
    {
       // alert("else");
         var id=document.getElementById(id).value;
         var radio_action=$("input[name='action_radio_all']:checked").val();
         var val1=$("#LeaveReasonscombo1").val();
         var a= $("#othercombo1").val();
        alert("second"+val1+a);
        if(typeof radio_action=="undefined")
    	{
    	 toasterrormsg("Please Select Action");
    ...
    //  alert(radio_action);   
           ...
           $.ajax({
                ...
                data: {id: id, status_flag: status_flag,remark: remark,planned:radio_action,reasoncombo:val1,reasontext:a},
                dataType: "json",
                 beforeSend: function(){
                loading();
            },
