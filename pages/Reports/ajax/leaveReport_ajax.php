...
if($_POST['type'] == "earlyDeparture"){
    ...
    $sql="select b.emp_name,a.* from (select EMP_CODE,CONVERT(VARCHAR(5), IN_TIME, 108) as IN_TIME,CONVERT(VARCHAR(5), OUT_TIME, 108) as OUT_TIME,CONVERT(VARCHAR(10), ATTDATE, 105) as ATTDATE from Cattendanceqry where CONVERT(varchar(10),ATTDATE,103) Between '$fDate' and '$tDate' ";

    if($empName != '')
    {
        $sql .="and EMP_CODE IN ($empName)";
    }
    if($scomp !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where COMP_CODE IN ($scomp))";
    }
    if($sbus !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where BussCode IN ($sbus))";
    }
    
    if($ssubbus !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where SUBBuss_Code IN ($ssubbus))";
    }
    if($sloc !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where LOC_CODE IN ($sloc))";
    }
    if($swloc !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where WLOC_CODE IN ($swloc))";
    }
    if($sfunc !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where FUNCT_CODE IN ($sfunc))";
    }
    if($ssubfunc !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where SFUNCT_CODE IN ($ssubfunc))";
    }
    if($scost !=''){
         $sql .="EMP_CODE IN (select emp_code from HrdMastQry where COST_CODE IN ($scost))";
    }if($sproc !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where PROC_CODE IN ($sproc))";
    }
    if($sgrade !=''){
        $sql .="EMP_CODE IN (select emp_code from HrdMastQry where GRD_CODE IN ($sgrade))";
    }
    if($sdes !=''){
         $sql .="EMP_CODE IN (select emp_code from HrdMastQry where DSG_CODE IN ($sdes))";
    }
    $sql .="and (CAST(ShiftMTo AS time)>= CAST(OUT_TIME AS time))) a join EmpNameQry b on a.EMP_CODE=b.Emp_Code";
