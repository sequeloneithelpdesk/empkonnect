...
complete: function() {
                ...
                popLoading();
                $("#calendar").removeClass('overlayBox');
                $(".loaderImage").hide();
}
...
$.extend(DayGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		if(event.type=='attendance'){
			...
			var stt = ''; if(event.status == 'Pending') stt = 'Leave-Pending'; else if(event.status == 'Approved') stt = ''; else stt = event.status;
			titleHtml += '<span class="att-time">'+stt+'</span>';
		}else if(event.type=='attRegularised'){
			var stt = ''; if(event.status == 'Pending') stt = 'Attn Regularised-Pending'; else stt = event.status;
			titleHtml += '<span class="att-time">'+stt+'</span>';
		}else if(event.type=='odrequest'){
				var stt = ''; if(event.status == 'Pending') stt = 'OD Request-Pending'; else stt = event.status;
				titleHtml += '<span class="att-time">'+stt+'</span>';		
